...
import { Radio, Eye, Heart, Gift, MessageCircle, X, MicOff, Mic, Volume2, Share2 } from 'lucide-react';
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', zIndex: 200, display: 'flex' }}>
      ...
      <div style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{
          ...
          background: `linear-gradient(135deg, hsl(${(stream.title?.charCodeAt(0) || 0) % 360}deg, 70%, 15%), hsl(${(stream.title?.charCodeAt(0) || 0) * 2 % 360}deg, 70%, 25%))`,
...
function GoLiveModal({ onClose }) {
  ...
  const startStream = async () => {
    ...
    const { data } = await supabase
      .from('live_streams')
      .insert({ user_id: user.id, title, category, status: 'live', viewer_count: 0, started_at: new Date().toISOString() })
      .select('*, user:profiles(*)')
      .single();
    if (data) {
      setStreaming(true);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="card" style={{ width: 480, padding: 24 }}>
        ...
        {streaming && <p style={{ marginTop: 12, fontSize: 13, color: '#22d3a5', textAlign: 'center' }}>Stream started!</p>}
      </div>
    </div>
  );
}
